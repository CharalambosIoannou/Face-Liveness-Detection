...
def occlude_convolution(final_img):
	c = 14
	l = []
		...
		if (i % (size_box+1) == 0):
			for j in range (height):
				if (j % (size_box+1) == 0):
					...
					occlude =  cv2.rectangle(img_c, (i, j), (i+size_box, j+size_box), (255, 0, 0), -1)
					final_img = cv2.rectangle(final_img, (i, j), (i+size_box, j+size_box), (255, 0, 0), -1)
					...
					if (label == 1):
						cv2.putText(final_img, perc, (i,j), cv2.FONT_HERSHEY_SIMPLEX ,
		                   (450 * 450) / (1000 * 1000) , (255, 255, 255) , 1, cv2.LINE_AA)
					else:
